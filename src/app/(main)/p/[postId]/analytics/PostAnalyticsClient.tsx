...
function calculateFallbackScore(
  engagement: { likes: number; replies: number; reposts: number; saves: number },
  followerCount: number,
  viewCount: number,
  ...
  const engagementScore = rawEngagement > 0 
    ? 100 * (1 - Math.exp(-rawEngagement / 20))
    : 0;
  
  const freshnessMultiplier = postAgeHours <= w.freshness.peakHours
    ? 1
    : Math.max(
        w.freshness.minimum,
        Math.pow(0.5, (postAgeHours - w.freshness.peakHours) / w.freshness.halfLifeHours)
      );
  ...
  const discoveryFactor = isNewCreator
    ? Math.max(
        1 - followerCount / w.discovery.followerThreshold,
        1 - accountAgeDays / w.discovery.ageDays,
        0
      )
    : 0;
  const discoveryScore = discoveryFactor * 100;
  
  const velocityFactor = postAgeHours > 0 && rawEngagement > 0
    ? Math.min(1, 1 - Math.exp(-rawEngagement / postAgeHours))
    : 0;
  ...
  const finalScore = Math.max(0, Math.min(100,
    weightedEngagement + weightedFreshness + weightedDiscovery + weightedVelocity + weightedAuthority - penalties
  ));
