      ...
      <div className="mb-5 flex items-start justify-between gap-4">
        ...
        {isOwner && (
          <div className="flex gap-2 shrink-0">
            <button onClick={onEdit} className="flex h-11 w-11 items-center justify-center rounded-lg border border-transparent text-white/45 transition-all hover:border-white/[0.1] hover:bg-white/[0.055] hover:text-white" title="Edit" aria-label={`Edit ${item.title}`}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                <path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7" />
                <path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z" />
              </svg>
            </button>
            <button onClick={onDelete} className="flex h-11 w-11 items-center justify-center rounded-lg border border-transparent text-white/45 transition-all hover:border-red-500/20 hover:bg-red-500/10 hover:text-red-400" title="Delete" aria-label={`Delete ${item.title}`}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
        ...
        {links.length > 0 && (
          <div className="flex items-center gap-3 shrink-0">
            {links.map((link, index) => {
              ...
              return (
                <a key={index} href={link} target="_blank" rel="noopener noreferrer" className="flex min-h-11 items-center gap-2 rounded-lg border border-white/[0.10] bg-white/[0.055] px-4 text-sm font-semibold text-white transition-all hover:border-white/[0.16] hover:bg-white/[0.085] active:scale-95">
                  {meta.icon}
                  {meta.label}
                </a>
              );
