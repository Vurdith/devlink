...
function renderEmailTemplate(options: {
                ...
                <td style="padding:24px">
                  <div style="border:1px solid rgba(255,255,255,0.08);background:#0f172a;border-radius:16px;overflow:hidden">
                    ...
                    <div style="padding:28px">
                      <h1 style="margin:0 0 6px 0;font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:22px;line-height:1.25;color:#ffffff">${safeHeading}</h1>
                      ${safeSubheading ? `<div style=\"margin:0 0 16px 0;color:#9aa4b2;font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-size:14px\">${safeSubheading}</div>` : ''}
                      ...
                      ${safeCtaLabel && safeCtaUrl ? `
                        <div style=\"margin:22px 0 4px 0\">
                          <a href=\"${safeCtaUrl}\" style=\"display:inline-block;background:linear-gradient(90deg,#7c3aed,#3b82f6);color:#ffffff;text-decoration:none;font-family:Inter,system-ui,-apple-system,Segoe UI,Roboto,Helvetica,Arial,sans-serif;font-weight:700;font-size:14px;padding:12px 18px;border-radius:10px;border:1px solid rgba(255,255,255,0.12)\">${safeCtaLabel}</a>
                        </div>
                        <div style=\"margin-top:12px;color:#93a4c7;font-family:ui-monospace,SFMono-Regular,Menlo,Monaco,Consolas,\'Liberation Mono\',\'Courier New\',monospace;font-size:12px;word-break:break-all\">
                          <a href=\"${safeCtaUrl}\" style=\"color:#93c5fd;text-decoration:none\">${safeCtaUrl}</a>
                        </div>
                      ` : ''}
                    </div>
                  </div>
  ...
  `.replace(/\u00c2\u00a9/g, "&copy;").replace(/\u00e2\u20ac\u00a2/g, "&middot;");
}
...
export async function sendEmailChangeVerification(newEmail: string, token: string): Promise<void> {
  ...
  const mailOptions = {
    ...
    html: renderEmailTemplate({
      ...
      body: `You requested to change your DevLink email address to <strong>${safeNewEmail}</strong>. Click the button below to verify this change.`,
