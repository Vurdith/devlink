...
function isDnsNotFoundError(e: unknown): boolean {
...
function disableUpstash(e: unknown) {
  if (upstashTemporarilyDisabled) return;
  upstashTemporarilyDisabled = true;
  console.warn("[RateLimit] Disabling Upstash Redis temporarily due to connectivity error.");
  if (process.env.NODE_ENV === "development") {
    console.warn("[RateLimit] Upstash error details:", e);
  }
  upstashClient = null;
}
