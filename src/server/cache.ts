...
function isUpstashDnsNotFoundError(e: unknown): boolean {
...
function disableUpstashWithReason(e: unknown) {
  if (upstashTemporarilyDisabled) return;
  upstashTemporarilyDisabled = true;
  console.warn("[Cache] Disabling Upstash Redis temporarily due to connectivity error.");
  if (process.env.NODE_ENV === "development") {
    console.warn("[Cache] Upstash error details:", e);
  }
  upstashClient = null;
}
  ...
  async set<T>(key: string, data: T, ttlSeconds: number = 300): Promise<void> {
    ...
    if (upstashClient) {
      try {
        ...
        if (isUpstashDnsNotFoundError(e)) {
          // We already log a single warning when we disable Upstash.
          disableUpstashWithReason(e);
        } else {
